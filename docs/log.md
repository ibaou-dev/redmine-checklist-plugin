...
## 2026-07-02 — v1.3.0 (checklist assignee & due in issue queries)

- **Assignee fold (default on, kill-switch).** `IssueQueryPatch#sql_for_assigned_to_id_field` overrides the built-in assignee filter so `assigned_to_id` also matches issues with a (non-section) checklist item assigned to the value — for every operator: `=`→`issue OR in_cl`, `!`→`issue AND NOT in_cl`, `*`→`issue OR any_cl`, `!*`→`issue AND NOT any_cl`. Reuses core `sql_for_field` for both the issue clause and the subquery's inner condition, so "me"/group expansion (done in `Query#statement` before dispatch) is honored. Gated by `RedmineChecklist.include_checklist_assignee?` (setting `include_checklist_assignee`, default on unless '0'). No new UI filter — it extends the EXISTING Assignee filter (that's the point: no shadow assignments). GOTCHA: `add_available_filter` label is a `:label` symbol key, not a resolved `name:` string.
- **"Checklist due" filter.** New `add_available_filter('checklist_due_date', type: :date, label: …)` + `sql_for_checklist_due_date_field` (subquery on open, non-converted items' due_date, reusing `sql_for_field` for date operators). Complements the sortable column from v1.2.0. NOT folded into core `due_date` (kept separate to avoid changing scheduling semantics).
- **"Checklist assignees" column** — `Issue#checklist_assignees` (distinct per-item users); the core Assignee column still shows the issue owner (filter includes checklist assignees; column = issue owner — a deliberate split). `format_object` renders the User array.
- **Migration 006** — index on `checklist_items.assignee_id` (backs the subqueries).
- **Checklist due dates derive the issue's `due_date` (combined with subtasks, default on).** `ChecklistItem.recalc_due_date(issue_id)` sets `issue.due_date = max(latest open non-converted checklist item due, subtask-derived due [children.maximum(:due_date) when core would derive])`, recomputed fresh, propagating upward; **only writes when there's a contribution** so removing the last source leaves the value sticky (mirrors core losing its last subtask — verified by `rails runner` against core behavior). Checklist items contribute **due only** (no start). Triggers: controller checklist changes (7 sites now call `ChecklistItem.recalc_issue` = done_ratio + due), plus `Issue` `after_save`/`after_destroy` hooks (`checklist_recalc_parent_due_date*`) on a child's `saved_change_to_due_date?`/`parent_id?` — core recomputes the subtask part on child save, our hook re-extends with the parent's checklist. Setting `combine_checklist_due` (default on). This is the pragmatic answer to "checklist deadlines in Calendar/Gantt": the issue really gets a `due_date`, so native views work — no core-widget patching. Edge cases verified: add/change/delete item, mark done (open-only), combined w/ subtask, subtask due change via hook, sticky-on-empty, kill-switch. Design decision: mirror subtask date derivation exactly (per [[project-status]]), don't reinvent.
- New `phase11-queries` spec (headline fold + kill-switch, column, due filter); all operators + kill-switch also verified at the query level via `rails runner`. Research/design captured in [[feature-checklist-in-queries]].
